...
public class Mesh3f extends VertexGroup {

    private int vao, pbo, ibo, cbo, tbo, nbo;

    public Mesh3f(Vertex3f[] vertices, int[] indices, Material material) {
    ...
    protected void createMesh() {
        ...
        for (int i = 0; i < vertices.length; i++) {
            normalData[i * 3] = ((Vertex3f) vertices[i]).getNormal().getX();
            normalData[i * 3 + 1] = ((Vertex3f) vertices[i]).getNormal().getY();
            normalData[i * 3 + 2] = ((Vertex3f) vertices[i]).getNormal().getZ();
        }
